...
const LuckyWheel = () => {
    ...
    const saveResult = async (prize) => {
        // 1. Decrement Stock
        await supabase.rpc('decrement_wheel_stock', { row_id: prize.id });
        // Note: Ideally create an RPC, but for now simple update if RLS allows, or just direct update
        // Since we don't have the RPC, let's try direct update (optimistic stock check handled before)
        ...
        if (prize.value !== 'NO_PRIZE') {
            ...
            // Trigger storage event for UrgencyBanner to pick up immediately
